...
def test_load_config_uses_bundled_defaults_without_local_config(tmp_path, monkeypatch):
    ...
    cfg = load_config()
    assert cfg["paths"]["output"] == "charts"
    icons_set = Path(cfg["paths"]["icons_set"])
    assert icons_set.exists()
    assert icons_set.name == "emoji"
    assert (icons_set / "dove.png").exists()
...
def test_bundled_default_icons_are_complete():
    """The default installed icon set must include all four birds."""
    src = REPO_ROOT / "icons" / "emoji"
    assert sorted(p.name for p in src.glob("*.png")) == ["dove.png", "eagle.png", "owl.png", "peacock.png"]
